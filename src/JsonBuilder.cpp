...
void JsonBuilder::process_message(smilelogmsg_t message) {	
	...
	if (temp.find("lld") != string::npos) {
	    ...
	    if (!count(this->feature_list.begin(),
			    this->feature_list.end(),
			    field)) {
...
void JsonBuilder::process_alignment_message(StreamingRecognizeResponse response,
	...
	for (int i = 0; i < result.alternatives_size(); i++) {
		...
		double start_time = this->sync_time + start_seconds +
				    (start_nanos / 1000000000.0);
		double end_time =
		...
		if (history.size() == 0) {
			...
			for (auto& it : history[0].items()) {
			    features_output[it.key()].push_back(
				entry[it.key()]);
			}
		    }
		}
...
void JsonBuilder::update_sync_time(double sync_time) { this->sync_time = sync_time; }
